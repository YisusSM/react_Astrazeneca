...
export default function ServiceInfo(params) {
  ...
  useEffect(() => {
    dispatch(getServiceInfo('Ronstring'))
  ...
  const drawHistoryItem = () => {
    if (!serviceInfo.history || serviceInfo.history.length === 0) {
      return (
        <>
          <div className="border-2 border-black flex flex-col overflow-x-auto">
    ...
    return serviceInfo.history.map((h) => {
      return (
        <>
          <div key={h.date} className="border-2 border-black flex flex-col overflow-x-auto">
            <div className="overflow-hidden bg-white shadow sm:rounded-lg">
              <div className="px-4 py-5 sm:px-6">
                ...
                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                  System status:{" "}
                  <span className={statusColor[h.system_status]}>{ serviceStatus[h.system_status] }</span>
                </p>
              </div>
                  ...
                  <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                      <ul className="list-disc">
                        {
                          serviceInfo.parents ? serviceInfo.parents.map((p, index) => {
                            return <li key={index}><a>{ p }</a></li>
                          }) : <></>
                        }
                      </ul>
                    </dd>
  ...
  return (
    <>
      <div className="bg-white px-6 lg:flex h-screen w-full flex-col lg:items-center lg:justify-between">
        ...
        <div className="mt-4 flex flex-col overflow-x-auto h-screen w-full">
          <div className="sm:-mx-6 lg:-mx-6">
            <div className="inline-block min-w-full py-2 sm:px-6 lg:px-8"></div>
              { drawHistoryItem() }
          </div>
        </div>
      </div>
